...
[XmlRoot(ElementName = "organisation", IsNullable = true, Namespace = "http://www.ech.ch/xmlns/eCH-0010-f/5")]
public class OrganisationMailAddressInfo : FieldValueChecker<OrganisationMailAddressInfo>
{
    ...
    public static OrganisationMailAddressInfo Create(string organisationName, string organisationNameAddOn1, string organisationNameAddOn2, eCH_0010_5_1.MrMrs? mrMr, string title, string firstName, string lastName)
    {
        return new OrganisationMailAddressInfo
        {
            ...
            MrMrs = (eCH_0010_5_1f.MrMrs)Enum.Parse(typeof(eCH_0010_5_1f.MrMrs), mrMr.ToString()),
            Title = title,
            FirstName = firstName,
            LastName = lastName
        };
    ...
    [JsonIgnore]
    [XmlIgnore]
    public bool OrganisationNameSpecified => !string.IsNullOrWhiteSpace(OrganisationName);
