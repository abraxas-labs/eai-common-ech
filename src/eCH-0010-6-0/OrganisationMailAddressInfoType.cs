...
[XmlRoot(ElementName = "organisation", IsNullable = true, Namespace = "http://www.ech.ch/xmlns/eCH-0010/6")]
public class OrganisationMailAddressInfoType : FieldValueChecker<OrganisationMailAddressInfoType>
{
    [JsonIgnore]
    [XmlNamespaceDeclarations]
    public XmlSerializerNamespaces Xmlns = new();

    private const string OrgValidationExMsg = "OrganisationMailAddressInfo is not valid! LastName cannot be null or empty if MrMrs, Title or FirstName are heaving a value";
    ...
    public OrganisationMailAddressInfoType()
    {
    ...
    public static OrganisationMailAddressInfoType Create(string organisationName, string organisationNameAddOn1, string organisationNameAddOn2, MrMrsType? mrMrs, string title, string firstName, string lastName)
    {
        if (string.IsNullOrEmpty(organisationName) && (!string.IsNullOrWhiteSpace(organisationNameAddOn1)
                                                        || !string.IsNullOrWhiteSpace(organisationNameAddOn2)
                                                        || mrMrs != null
                                                        || !string.IsNullOrEmpty(title)
                                                        || !string.IsNullOrEmpty(firstName)
                                                        || !string.IsNullOrEmpty(lastName)))
        {
            throw new FieldValidationException(OrgValidationExMsg);
        }
    ...
    /// <param name="organisationName">Field is reqired.</param>
    /// <returns>OrganisationMailAddressInfo.</returns>
    public static OrganisationMailAddressInfoType Create(string organisationName)
    {
        return new OrganisationMailAddressInfoType
        {
            OrganisationName = organisationName,
